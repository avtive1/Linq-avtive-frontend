...
export default function VerifyEmailIncorrectPage() {
  ...
  const handleChange = (index: number, value: string) => {
    ...
    if (value && index < 3) {
      const nextInput = document.getElementById(`code-err-${index + 1}`)
      nextInput?.focus()
    }
  }
  ...
  const handleKeyDown = (index: number, e: React.KeyboardEvent) => {
    if (e.key === "Backspace" && !code[index] && index > 0) {
      const prevInput = document.getElementById(`code-err-${index - 1}`)
      prevInput?.focus()
    }
  }
      ...
      <div className="flex w-full lg:w-1/2 flex-col bg-white px-6 py-8 sm:px-12">
        ...
        <div className="flex flex-1 items-center justify-center">
          <div className="flex w-full max-w-md flex-col items-center gap-6">
            ...
            {/* Code Input - 4 separate boxes with RED error borders */}
            <div className="flex items-center gap-3">
              {code.map((digit, index) => (
                <input
                  ...
                  className="size-14 rounded-lg border-2 border-red-400 bg-red-50/50 text-center text-xl font-semibold text-gray-800 outline-none focus:border-red-500 focus:ring-2 focus:ring-red-200"
                />
              ))}
            </div>
